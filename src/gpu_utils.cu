...
void allocGPUInterpList(INTERPLIST** d_interplist, int ninterp, INTERPLIST* interplist) {
  ...
  HANDLE_ERROR(cudaMemcpy((*d_interplist), tmplist, ninterp * sizeof(INTERPLIST), cudaMemcpyHostToDevice));
...
void updateSolnGPU(int nrecv, PACKET *rcvPack, GPUvec<double> *vec) {
  ...
  HANDLE_ERROR(cudaMemcpy(d_idata, h_idata, count * sizeof(int), cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(d_ddata, h_ddata, count * nvar * sizeof(double), cudaMemcpyHostToDevice));
